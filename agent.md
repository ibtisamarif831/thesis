...
- The dashboard currently supports:
  ...
  - selected icon details and cluster summaries, whose expanded panels can open every currently filtered cluster icon in a fullscreen gallery;
  ...
  - manual-style AI Clustering inspection with synchronized method/k controls, selected-icon seven-feature detail, per-cluster characteristic measured features, fullscreen cluster galleries, and a post-hoc seven-feature between-cluster separation ranking; the paired feature-vs-AI plots and agreement cross-table open from a dedicated fullscreen comparison button. The ranking is descriptive and must not be called model attribution because image embeddings receive pixels only.
