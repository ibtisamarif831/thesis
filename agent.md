...
- The dashboard currently supports:
  ...
  - a collapsible left Clustering controls sidebar and a compact right-side cluster launcher: **Show heatmap** and **View all cluster details** open fullscreen modal views instead of rendering the heatmap and cluster panels inline; the all-details modal expands every cluster with its complete currently filtered icon set;
  ...
  - manual-style AI Clustering inspection with synchronized method/k controls, a collapsible analysis sidebar, selected-icon seven-feature detail, modal-only heatmap/all-cluster details, and a post-hoc seven-feature between-cluster separation ranking; the paired feature-vs-AI plots and agreement cross-table open from a dedicated fullscreen comparison button. The ranking is descriptive and must not be called model attribution because image embeddings receive pixels only.
