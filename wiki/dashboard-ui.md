...
The fifth top-level view compares feature clustering with learned image-embedding clustering for the exact current seven-family composite used by Image Clustering. Start `.venv/bin/python code/serve_analysis_dashboard.py --port 8765` and open `http://127.0.0.1:8765/`. Put `OPENROUTER_API_KEY` in the ignored repository-root `.env` file or server environment. If it is absent, normal dashboard views remain usable and **Run AI Clustering** is disabled with a configuration message.

The button is the only action that may spend provider credits. Randomizing a sample or changing feature selection, method, or `k` marks the visible result stale and never triggers an automatic request. If only two image features are selected, the next explicit run recomputes the feature-based PCA and labels from those two features. The AI side still uses the full learned image embeddings for the same icon sample; feature selection does not enter the embedding request or alter a cached embedding. Both plots retain the run's same ordered icon IDs and synchronize hover. The panel reports model, sample, cache counts, pairwise agreement, and a feature-versus-AI cross-table. ARI, NMI, and provider usage remain stored with the run but are intentionally hidden from the current UI. These are agreement measurements, not proof that either clustering is better.

Recent runs persist in SQLite. **Load** restores the stored plots and metrics without calling OpenRouter and without mutating the current Feature Groups sample. Version one intentionally has no cluster names or “why” explanations.
