...
- **Color by:** control is populated with cluster, icon set, and the seven code-selected analysis features.
...
The sidebar exposes only the seven features in `metadata.image_feature_columns`, one from each family. All seven are checked on load and are the only features browser-side PCA and clustering will accept. The 81-feature registry remains available in metadata and other inspection views, but cannot leak into the Clustering selection. Available actions:
...
The main view stays concise. It presents the seven human-facing categories and one representative selector for each family. The configured choices remain the documented study defaults. Choosing another active registry feature in a family is a browser-session exploratory override for Feature Groups only; it does not modify the code-selected seven-feature Clustering allow-list. Reloading restores the configured defaults. Each family row also has a **View details** button that opens a full-viewport detail workspace containing:
...
- up to 20 dataset-balanced icons drawn independently for that family from the 28,260 icons with a certain foreground mask;
...
8. Change Complexity from Edge density to Quadtree leaf count. Return to Clustering and confirm the seven code-selected checkboxes are unchanged, Edge density remains the only enabled Complexity feature there, and the K-means plot and summaries still render.
