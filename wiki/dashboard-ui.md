...
The all-details modal also lists standardized distinctive families/features and every currently visible icon for every cluster; there is no twelve-icon sidebar cap or second gallery action. Close it with its close button or **Escape**; focus returns to the right-sidebar launcher. All interpretations use the feature space directly responsible for manual Image clustering and remain descriptive rather than causal.
...
15. In Image Clustering, collapse and restore the left sidebar and confirm the plot resizes. Confirm the right sidebar contains only **Show heatmap** and **View all cluster details** for cluster analysis. Open the heatmap and verify one row per cluster and one column per selected feature. Open all details and confirm every cluster starts expanded, variance contributions total approximately 100% after rounding, six icon statistics and one feature-statistics row per selected feature appear, and the modal includes every currently visible icon. After changing the icon-set filter, only **Currently visible** and the modal icon sets change while full cluster statistics remain stable.
16. In AI Clustering, collapse and restore the analysis sidebar and confirm the embedding plot resizes. Load a saved run without clicking **Run AI Clustering**. Confirm its cluster-summary section contains only the two modal launchers. Confirm the heatmap has seven representative columns and the all-details modal reports separate embedding and measured contribution/strength values, six icon statistics, seven feature-statistics rows, and all icons. Both contribution columns should total approximately 100%, and no provider request should occur. For an intentionally cache-missing old run, confirm embedding metrics say **Unavailable** rather than falling back to two-dimensional PCA.
...
The AI tab exposes synchronized Method and Cluster count controls, one large image-embedding plot, and a collapsible analysis sidebar containing selected-icon details, the measured-feature ranking, and two compact cluster-analysis launchers. **Hide sidebar** releases the full workspace width for the AI plot; **Show sidebar** restores it and the plot resizes after either action. AI hover and click use the loaded run's icon set as the global baseline and show the same compact feature value/comparison bars as manual Clustering. **Show heatmap** opens the seven-feature profile in the shared fullscreen modal. **View all cluster details** opens every AI cluster, already expanded, with all icons. Its plot icons use the same 5.5%-of-axis-span scale as manual Image clustering, including both plots in the comparison modal. Image and AI cluster-detail cards share one renderer and the same ordering: **Variance contribution**, **Separation strength**, **Full cluster size**, icon statistics, feature statistics, interpretation, and icons. In AI, the first two metrics are calculated in the model's full embedding space; additional **Post-hoc measured variance** and **Post-hoc measured strength** values describe how the seven thesis features align with those AI clusters. The feature-statistics table remains identical, with valid `N`, raw cluster mean, median, population standard deviation, minimum, maximum, full-sample mean, and signed standardized difference. These measured-feature statistics are post-hoc descriptions of the AI clusters, not embedding dimensions or model attribution. Clicking a plot icon populates the same kind of compact measurement detail used by manual Clustering. AI retains its explicit Run and Recent Runs controls because embedding requests and saved results require an explicit lifecycle.
