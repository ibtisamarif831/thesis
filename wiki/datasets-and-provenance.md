...
The current `features.csv` covers all 28,749 canonical rows across all 13 sets. The verified run used neither `--limit` nor `--per-set-limit`, and `feature_failures.json` is empty. Feature Groups applies the documented technical-quality gate and retains 28,260 rows: all 489 exclusions are uncertain foreground masks, while the current corpus has no missing or non-finite representative values. The generated `feature_group_quality_audit.json` records those counts plus diagnostic low-tail candidates; low scores alone cause no exclusion. The eligible pool supplies transient equal-width stratified family samples and three-icon comparisons. Image and AI Clustering use the unique composite of all seven family samples, normally 140 icons for the All cohort. The separate deterministic 129-row generated sample remains authoritative only for Metadata/Combined projections, as described in [Dashboard implementation](dashboard-implementation.md).
